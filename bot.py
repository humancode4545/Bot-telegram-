...
async def cmd_resetwarn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Solo owners con sesión activa pueden resetear advertencias."""
    if not is_owner(update, context):
        await update.message.reply_text(
...
async def handle_private_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Handler unificado para texto en chat privado.
    Orden: 1) palabra secreta → 2) pending_action → 3) ignorar
    """
    msg = update.message
    if not msg or not msg.text or not msg.from_user:
        return
    ...
    # 1) Verificar palabra secreta primero
    if text == keyword:
        if uid in owners:
        ...
        # Si no es owner: ignorar silenciosamente
        return

    # 2) Si hay acción pendiente, pasarlo a handle_admin_reply
    if context.user_data.get("pending_action") and uid in owners:
        await handle_admin_reply(update, context)

# ── Media privada (imágenes, audio, docs del admin) ────
async def handle_admin_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "wp_capture_image":
        if not msg.photo:
            context.user_data["pending_action"] = "wp_capture_image"
            await msg.reply_text("⚠️ Debes enviar una *imagen*. Intenta de nuevo:",
                                 parse_mode="Markdown")
            return
        file_id = msg.photo[-1].file_id
        data.setdefault("welcome_config", {})["image_file_id"] = file_id
        save_data(data)
        context.user_data.pop("pending_action", None)
        await msg.reply_text("✅ Imagen de bienvenida guardada correctamente.")
        return

    # Para otros tipos de media (contenido del Centro de Mensajes)
    await handle_admin_reply(update, context)
def main():
    app = (
        ApplicationBuilder()
    ...
    # ── Moderación en grupos ───────────────────────────
    app.add_handler(MessageHandler(
        (filters.TEXT & ~filters.COMMAND) & (filters.ChatType.GROUP | filters.ChatType.SUPERGROUP),
        moderate_text))
    app.add_handler(MessageHandler(
        (filters.PHOTO | filters.VIDEO | filters.VIDEO_NOTE) & (filters.ChatType.GROUP | filters.ChatType.SUPERGROUP),
        moderate_media))
    app.add_handler(MessageHandler(
        filters.Sticker.ALL & (filters.ChatType.GROUP | filters.ChatType.SUPERGROUP),
        moderate_sticker))

    # ── Chat privado — texto unificado ───────────────────
    app.add_handler(MessageHandler(
